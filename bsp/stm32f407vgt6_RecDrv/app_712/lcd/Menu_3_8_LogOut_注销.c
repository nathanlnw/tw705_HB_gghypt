...
{
lcd_fill(0);
lcd_text12(0, 3,"清空鉴权码    鉴权",18,LCD_MODE_SET);
lcd_text12(0,18,"注册          注销",18,LCD_MODE_SET);
if(par==1)
	lcd_text12( 0,  3,"清空鉴权码",10,LCD_MODE_INVERT);
else if(par==2)
	lcd_text12(84,  3,"鉴权",4,LCD_MODE_INVERT);
else if(par==3)
	lcd_text12( 0, 18,"注册",4,LCD_MODE_INVERT);
else if(par==4)
	lcd_text12(84, 18,"注销",4,LCD_MODE_INVERT);
lcd_update_all();
}
...
static void keypress(unsigned int key)
{
   ...
   switch(KeyValue)
	   ...
	   case KeyValueOk:
	   	   if(LogInorOut==1)//清空鉴权码
		   ...
		   else if(LogInorOut==4)//注销
			   {
			   LogInorOut_screen=0;
			   LogInorOut=0;
			   lcd_fill(0);
			   lcd_text12(0,10,"按确认键发送车台注销",20,LCD_MODE_SET);
			   lcd_update_all();
			   Menu_Logout=4;
			   }
			   ...
			   Menu_Logout=0;
				...
				DEV_regist.DeRegst_sd=1; // set 发送注册标志位

			   lcd_fill(0);
			   lcd_text12(30,10,"注销已发送",10,LCD_MODE_SET);
			   lcd_update_all();
			   }
		   break;
	   case KeyValueUP:
